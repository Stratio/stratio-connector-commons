...
public abstract class CommonsMetadataEngine<T> implements IMetadataEngine {
    ...
    public final void dropIndex(ClusterName targetCluster, IndexMetadata indexMetadata) throws UnsupportedException,
        ...
        try {
            connectionHandler.startJob(targetCluster.getName());
            if (logger.isDebugEnabled()) {
                logger.debug("Dropping index [" + indexMetadata.getName().getName() + "] in cluster [" + targetCluster
                        .getName() + "]");
            }
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Index [" + indexMetadata.getName().getName() + "] has been drepped successfully in cluster ["
                                + targetCluster.getName() + "]");
            }
        } finally {
            connectionHandler.endJob(targetCluster.getName());
    ...
    public List<CatalogMetadata> provideMetadata(ClusterName targetCluster) throws ConnectorException {
        try {
            ...
            List<CatalogMetadata> metadata = provideMetadata(targetCluster,
    ...
    public CatalogMetadata provideCatalogMetadata(ClusterName targetCluster, CatalogName catalogName)
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Providing catalog [" + catalogName.getName() + "] metadatada from cluster [" + targetCluster
                                .getName() + "]");
            }
    ...
    public TableMetadata provideTableMetadata(ClusterName targetCluster, TableName tableName)
        ...
        try {
            connectionHandler.startJob(targetCluster.getName());
            if (logger.isDebugEnabled()) {
                logger.debug("Providing table [" + tableName.getName() + "] metadatada from cluster [" + targetCluster
                        .getName() + "]");
            }
            ...
            TableMetadata tableMetadata = provideTableMetadata(tableName, targetCluster,
                    connectionHandler.getConnection(targetCluster.getName()));
            if (logger.isDebugEnabled()) {
                logger.debug("Metadatada  from [" + tableName.getName() + "] has been provided from cluster ["
                        + targetCluster.getName() + "]");
            }
